...
static icu::Locale getCLocale() {
	icu::Locale c_locale = Unicode::getICULocale(
...
TEST(TestStringUtils, EqualsNoCase_Normalize) {
	...
	EXPECT_TRUE(
			StringUtils::EqualsNoCase(refstr, varstr,
					StringOptions::FOLD_CASE_DEFAULT, true));
}
...
TEST(TestStringUtils, FoldCase_W) {
	std::wstring w_s1 = Unicode::utf8_to_wstring(
			std::string(UTF8_MULTI_CODEPOINT_CHAR1_VARIENT_5));
	std::wstring w_s2 = Unicode::utf8_to_wstring(
			std::string(UTF8_MULTI_CODEPOINT_CHAR1_VARIENT_1));
	StringUtils::FoldCase(w_s1);
	StringUtils::FoldCase(w_s2);
	int32_t result = StringUtils::Compare(w_s1, w_s2);
	EXPECT_NE(result, 0);

	w_s1 = Unicode::utf8_to_wstring(
			std::string(UTF8_MULTI_CODEPOINT_CHAR1_VARIENT_5));
	w_s2 = Unicode::utf8_to_wstring(
			std::string(UTF8_MULTI_CODEPOINT_CHAR1_VARIENT_1));
...
TEST(TestStringUtils, Normalize) {
	...
	StringUtils::Normalize(s1, StringOptions::FOLD_CASE_DEFAULT,
			NormalizerType::NFD);
	int result = StringUtils::Compare(s1, UTF8_MULTI_CODEPOINT_CHAR1_VARIENT_2);
	EXPECT_EQ(result, 0);

	s1 = UTF8_MULTI_CODEPOINT_CHAR1_VARIENT_1;
	StringUtils::Normalize(s1, StringOptions::FOLD_CASE_DEFAULT,
			NormalizerType::NFC);
	 result = StringUtils::Compare(s1, UTF8_MULTI_CODEPOINT_CHAR1_VARIENT_5);
	EXPECT_EQ(result, 0);
	...
	StringUtils::Normalize(s1, StringOptions::FOLD_CASE_DEFAULT,
			NormalizerType::NFD);
	 result = StringUtils::Compare(s1, UTF8_MULTI_CODEPOINT_CHAR1_VARIENT_2);
	EXPECT_EQ(result, 0);
...
TEST(TestStringUtils, Split) {
	...
	EXPECT_STREQ("a bc",
			StringUtils::Split("a bc  d ef ghi ", "  ", 10).at(0).c_str());

	EXPECT_EQ(1U, StringUtils::Split("a bc  d ef ghi ", " z").size());
	EXPECT_STREQ("a bc  d ef ghi ",
			StringUtils::Split("a bc  d ef ghi ", " z").at(0).c_str());

	EXPECT_EQ(1U, StringUtils::Split("a bc  d ef ghi ", "").size());
	EXPECT_STREQ("a bc  d ef ghi ",
			StringUtils::Split("a bc  d ef ghi ", "").at(0).c_str());

	// test overload with char as delimiter
	EXPECT_EQ(4U, StringUtils::Split("a bc  d ef ghi ", ' ', 4).size());
	EXPECT_STREQ("d ef ghi ",
			StringUtils::Split("a bc  d ef ghi ", ' ', 4).at(3).c_str());
	EXPECT_EQ(7U, StringUtils::Split("a bc  d ef ghi ", ' ').size())
			<< "Result must be 7 strings including two empty strings";
	EXPECT_STREQ("bc", StringUtils::Split("a bc  d ef ghi ", ' ').at(1).c_str());
	EXPECT_STREQ("", StringUtils::Split("a bc  d ef ghi ", ' ').at(2).c_str());
...
TEST(TestStringUtils, Collate) {
	...
	var = StringUtils::Collate(s1, s2, getUSEnglishLocale());
	EXPECT_GT(var, ref);
	...
	var = StringUtils::Collate(s3, s4, getTurkicLocale(), true);  // Normalize
	EXPECT_EQ(var, 0);

	var = StringUtils::Collate(s3, s4, getTurkicLocale(), false); // No (extra) Normalization
	EXPECT_NE(var, 0);
}
...
TEST(TestStringUtils, TimeStringToSeconds) {
	...
	EXPECT_EQ(2 * 3600 + 4 * 60 + 3,
			StringUtils::TimeStringToSeconds("   2:4:3"));
	EXPECT_EQ(2 * 3600 + 4 * 60 + 3,
			StringUtils::TimeStringToSeconds("  \t\t 02:04:03 \n "));
	EXPECT_EQ(1 * 3600 + 5 * 60 + 2,
			StringUtils::TimeStringToSeconds("01:05:02:04:03 \n "));
...
TEST(TestStringUtils, FindWords) {
	...
	EXPECT_EQ(ref, var);
	var = StringUtils::FindWords("anyt]h(z_iILi234#!? 1a34#56bbc7 ",
			"1a34#56bbc7");
	ref = 20;
	EXPECT_EQ(ref, var);
}
...
TEST(TestStringUtils, FindWords_NonAscii) {
	...
	ref = 11;
	var = StringUtils::FindWords("abcçdefgğh ıİi jklmnoöprsştuüvyz",
			"ıiİ jklmnoöprsştuüvyz");
}
...
TEST(TestStringUtils, FileSizeFormat) {
	...
	EXPECT_STREQ("5432PB",
			StringUtils::FormatFileSize(6115888293969133568).c_str());
}
