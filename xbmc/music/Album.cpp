    ...
    for (size_t i = 0; i < mbids.size(); i++)
    {
      ...
      */
      ...
      if (artistSort.size() == mbids.size())
	  {
        ...
        artistCredits.emplace_back(UnicodeUtils::Trim(artistName), artistSort[i], artistId);
	  }
      else
...
bool CAlbum::Load(const TiXmlElement *album, bool append, bool prioritise)
{
  ...
  strReleaseDate = UnicodeUtils::Trim(strReleaseDate);  // @todo: validate ISO8601 format
